...
function getReady(phase , subPhase)
{
    ...
    $('.submitWithVerb').click(function(e)
    {
        ...
        else
        {
            ...
            // Dynamic sections - should be put in the json only if they are visible.
            $dynamicSections = {} ;
            $('.dynamicSectionsWrapper').each(function() {
                var thisWrapper = $(this) ;
                // The otherBusinessSection has 'display:table' if it is shown and 'display:none' if it's not.
                if (thisWrapper.closest('.otherBusinessSection').css('display') === 'table')
                {
                    var sectionNumber = 1 ;
                    var wrapperContent = {} ;
                    // each dynamicSection will have local-postable items
                    thisWrapper.find('.dynamicSection').each(function(){
                        var thisSection = $(this) ;
                        var sectionContent = {} ;
                        thisSection.find('.local-postable').each(function(key,value){
                           sectionContent[$(this).attr('name')]=$(this).val(); 
                        });
                        wrapperContent[sectionNumber++] = sectionContent ;
                    }) ;
                    $dynamicSections[thisWrapper.attr('name')] = wrapperContent ;
                }
            });
            /*
            var i = 1;
            $('.dynamicSectionsWrapper').find('.dynamicSection').each(function(key,value) {
                var thisSection = $(this) ;
                $dynamicSections[i++] = 'A dynamic section';
            });
            */
            if (Object.keys($dynamicSections).length>0)
            {
                json['dynamicSections'] = $dynamicSections ;
            }
        }
    ...
    // Prepare popovers to shown Loyal to whom when hovering on unit
    $('.globalRomeCurrentState').on('shown.bs.popover', function(){
        $('.unit').popover();
    });
...
{
    // The Twig template for otherBusiness must have a wrapper <div> of class .otherBusinessWrapper with all showable/hideable sections.
    ...
    $('.otherBusinessList').on('change', function()
    {
        ...
        $('.otherBusinessSection').each(function(i){
            ...
            {
                $(this).show();
                // Populate the section that is shown
    ...
    // We need this to get the cost of legions and fleets, so we can dynamically display the total cost
...
/**
 * A dynamic section is a section in which items can be added and removed dynamically, e.g governorship
 */
function prepareDynamicSection()
{
    $('.dynamicSectionAddButton').click(function () {
        
        var $boxToClone = $('.dynamicSection:first', $(this).closest('.dynamicSectionsWrapper'));
    ...
    $('.dynamicSectionsWrapper').on('click', '.dynamicSectionRemoveButton', function (e) {
        if ($('.dynamicSection', $(this).closest('.dynamicSectionsWrapper')).length > 1)
        {
...
function senateOtherBusinessPopulateSection($otherBusinessType)
{
    ...
    // Go through this otherBusinessType section, and add 'global-postable'to all 'local-postable' elements
    // I don't do that for dynamic sections, as they will be handled on submit
    // TO DO : Isn't there a problem if it's a mix of dynamic sections (in which local-postable is handled separately) and non dynamic sections (which should absolutely be made global-postable)
    var hasDynamicSection = $('#'+$otherBusinessType).find('.dynamicSectionsWrapper').length>0 ;
    if (!hasDynamicSection)
    {
        $('#'+$otherBusinessType).find('.local-postable').each(function(key,value) {
           $(this).addClass('global-postable');
        });
    }
    ...
    $('.sprite-Senator').each(function(i, senator) {
        ...
        if ($.inArray($otherBusinessType , $otherBusinessList) > -1) 
        {
            ...
            {
                $senatorFullName+=' ('+$json.office+') MIL: '+$json.MIL;
    ...
    if ($otherBusinessType=='concession' || $otherBusinessType=='commander')
    {
        ...
        $('.sprite-Card').each(function(i) {
            ...
            // Append (deck name) after the card name
            var $json = $(this).data('json') ;
            var $cardName = $json.name+ ' ('+$json.deck+')' ;
            var $card_id = $json.card_id ;
            var $otherBusinessList = $json.OtherBusiness ;
            // If the $otherBusinessType is in this Card's $otherBusinessList, add his {value,text} to the #otherBusinessCardSelect{$otherBusinessType}
            if ($.inArray($otherBusinessType , $otherBusinessList) > -1) 
            {
